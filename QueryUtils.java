...
public final class QueryUtils {
    ...
    private static List<News> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            JSONArray newsDetailsArray = responseNews.getJSONArray("results");

            // For each earthquake in the earthquakeArray, create an {@link Earthquake} object
            for (int i = 0; i < newsDetailsArray.length(); i++) {

                // Get a single earthquake at position i within the list of earthquakes
                JSONObject currentNews = newsDetailsArray.getJSONObject(i);
                ...
                if(tagsArray.length() > 0) {
                    ...
                    authorName = tagsFirstObject.getString("webTitle");
                }


                // Create a new {@link News} object from the JSON response.

                News news_item = new News(webTitle, authorName, sectionName, webPublicationDate, url);
